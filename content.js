...
CONTENT.uk = {
  ...
  tabs: { plasma:'Плазма', laser:'Лазер', water:'Воднострумінь', oxy:'Кисень', mill:'Фрезерування', common:'Спільне', faults:'Несправності', gloss:'Словник', iv:'FAQ', gcode:'G-код', tdocs:'Документація', mat:'Матеріали' },
    ...
    plasma: { color:'var(--arc)', eyebrow:'Термічне різання · електрична дуга', h1:'Плазма',
      ...
      sections: [
        ...
        { n:'06', h2:'Гази', blocks:[
          ...
          {type:'p', html:'Газ визначає не лише якість кромки, а й <b>матеріал емітера в електроді</b>. Логіка одна: чи є в газі кисень.'},
          {type:'table', head:['Газ','Матеріал емітера'], rows:[
            ['Кисень','<b>гафній</b>'],
            ['Повітря','<b>гафній</b> (або цирконій)'],
            ['Азот','<b>вольфрам</b>'],
            ['H35 · Ar + H₂','<b>вольфрам</b>'],
            ['Аргон','<b>вольфрам</b>'] ]},
          {type:'p', html:'Вольфрам сам по собі кращий емітер — плавиться близько 3400 °C проти 2200 °C у гафнію і служить довше. Але він <b>миттєво окислюється</b>. Гафній гірший як емітер, зате утворює тугоплавкий оксид HfO₂ (≈ 2750 °C), який витримує процес. Тобто <b>гафній — це вимушений компроміс заради кисню</b>, а де кисню немає, ставлять вольфрам.'},
          {type:'note', html:'Увага: <b>повітря — це теж кисень</b>, приблизно 21 %. Тому повітряна плазма, найпоширеніший режим узагалі, теж вимагає гафнію.'},
          {type:'callout', hd:'Помилка, яка виглядає як брак', p:'Вольфрамовий електрод у повітряній плазмі згорає майже одразу. Клієнт дзвонить: «електроди дохнуть за п’ять хвилин, у вас брак» — а насправді взяли не той комплект. Зворотна помилка м’якша: гафній в азоті працює, але зношується швидше, ніж мав би вольфрам, і клієнт просто переплачує. Загальне правило: <b>витратники підбираються комплектом під конкретний струм І конкретний газ</b>.'} ]} ]},
...
CONTENT.sk = {
  ...
  tabs: { plasma:'Plazma', laser:'Laser', water:'Vodný lúč', oxy:'Autogén', mill:'Frézovanie', common:'Spoločné', faults:'Poruchy', gloss:'Slovník', iv:'FAQ', gcode:'G-kód', tdocs:'Dokumentácia', mat:'Materiály' },
    ...
    plasma: { color:'var(--arc)', eyebrow:'Tepelné rezanie · elektrický oblúk', h1:'Plazma',
      ...
      sections: [
        ...
        { n:'06', h2:'Plyny', blocks:[
          ...
          {type:'p', html:'Plyn určuje nielen kvalitu hrany, ale aj <b>materiál emitora v elektróde</b>. Logika je jediná: či je v plyne kyslík.'},
          {type:'table', head:['Plyn','Materiál emitora'], rows:[
            ['Kyslík','<b>hafnium</b>'],
            ['Vzduch','<b>hafnium</b> (alebo zirkónium)'],
            ['Dusík','<b>volfrám</b>'],
            ['H35 · Ar + H₂','<b>volfrám</b>'],
            ['Argón','<b>volfrám</b>'] ]},
          {type:'p', html:'Volfrám je sám osebe lepší emitor — taví sa okolo 3400 °C oproti 2200 °C hafnia a vydrží dlhšie. Lenže sa <b>okamžite oxiduje</b>. Hafnium je horší emitor, zato tvorí žiaruvzdorný oxid HfO₂ (≈ 2750 °C), ktorý proces znesie. Inak povedané, <b>hafnium je nutný kompromis kvôli kyslíku</b>, a kde kyslík nie je, dáva sa volfrám.'},
          {type:'note', html:'Pozor: <b>vzduch je tiež kyslík</b>, približne 21 %. Preto aj vzduchová plazma, vôbec najrozšírenejší režim, vyžaduje hafnium.'},
          {type:'callout', hd:'Chyba, ktorá vyzerá ako nepodarok', p:'Volfrámová elektróda vo vzduchovej plazme zhorí takmer okamžite. Zákazník volá: „elektródy vydržia päť minút, máte zlý tovar" — pritom si len vzal nesprávnu sadu. Opačná chyba je miernejšia: hafnium v dusíku funguje, ale opotrebuje sa rýchlejšie než volfrám, a zákazník zbytočne preplatí. Všeobecné pravidlo: <b>spotrebný materiál sa vyberá ako sada na konkrétny prúd A konkrétny plyn</b>.'} ]} ]},
...
CONTENT.en = {
  ...
  tabs: { plasma:'Plasma', laser:'Laser', water:'Waterjet', oxy:'Oxy-fuel', mill:'Milling', common:'Common', faults:'Faults', gloss:'Glossary', iv:'FAQ', gcode:'G-code', tdocs:'Documentation', mat:'Materials' },
    ...
    plasma: { color:'var(--arc)', eyebrow:'Thermal cutting · electric arc', h1:'Plasma',
      ...
      sections: [
        ...
        { n:'06', h2:'Gases', blocks:[
          ...
          {type:'p', html:'The gas determines not only edge quality but also the <b>emitter material in the electrode</b>. The logic is simple: is there oxygen in the gas.'},
          {type:'table', head:['Gas','Emitter material'], rows:[
            ['Oxygen','<b>hafnium</b>'],
            ['Air','<b>hafnium</b> (or zirconium)'],
            ['Nitrogen','<b>tungsten</b>'],
            ['H35 · Ar + H₂','<b>tungsten</b>'],
            ['Argon','<b>tungsten</b>'] ]},
          {type:'p', html:'Tungsten is the better emitter in its own right — it melts at around 3400 °C against hafnium\'s 2200 °C and lasts considerably longer. But it <b>oxidises instantly</b>. Hafnium is the weaker emitter, yet it forms a refractory oxide, HfO₂ (≈ 2750 °C), that survives the process. In other words <b>hafnium is a compromise forced on you by oxygen</b>, and where there is no oxygen you fit tungsten instead.'},
          {type:'note', html:'Note that <b>air is oxygen too</b> — roughly 21 %. So air plasma, the most common mode of all, also requires hafnium.'},
          {type:'callout', hd:'The mistake that looks like a defect', p:'A tungsten electrode in air plasma burns out almost immediately. The customer calls to say "your electrodes last five minutes, they must be faulty" — when in fact the wrong set was fitted. The reverse mistake is gentler: hafnium in nitrogen works, but wears faster than tungsten would, and the customer simply overpays. The general rule: <b>consumables are selected as a set, for a specific current AND a specific gas</b>.'} ]} ]},
